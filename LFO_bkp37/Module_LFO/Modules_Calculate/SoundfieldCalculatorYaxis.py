...
class SoundFieldCalculatorYaxis(ModuleBase):
    ...
    def calculate_sound_field_column(self, column_index):
        ...
        Berechnet eine einzelne Spalte des Schallfelds (konstante X-Position).
        Nutzt Batch-Interpolation für optimale Performance.
        
        Args:
            column_index: Index der X-Position im Grid
            
        Returns:
            tuple: (sound_field_p, sound_field_x, sound_field_y)
        """
        # ============================================================
        # SCHRITT 1: Grid-Erstellung (1D - nur Y-Achse)
        # ============================================================
        ...
        # Erstelle Koordinatenraster nur für eine Spalte (konstante X-Position)
        sound_field_x, sound_field_y = np.meshgrid(np.array([sound_field_y[column_index]]), sound_field_x)
        ...
        for speaker_array in self.settings.speaker_arrays.values():
            ...
            for isrc in range(len(source_indices)):
                ...
                # --------------------------------------------------------
                # Berechne Distanzen für ALLE Punkte der Y-Achse gleichzeitig
    ...
    def _interpolate_angle_data_batch(self, magnitude, phase, vertical_angles, azimuths, elevations):
        ...
        Kernfunktion für die Performance-Optimierung der Y-Achsen-Berechnung.
