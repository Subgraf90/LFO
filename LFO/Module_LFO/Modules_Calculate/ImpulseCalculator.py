...
class ImpulseCalculator(ModuleBase):
    ...
    def calculate_phase_response(self, impulse_responses, balloon_cache=None):
        ...
        for point_key in self.settings.impulse_points:
            ...
            if air_absorption_coeffs is not None:
                attenuation = np.exp(-air_absorption_coeffs[None, :] * distance_array)
            else:
                attenuation = 1.0
            mag_with_gain = (mag_stack / distance_safe) * gain_linear * attenuation
    ...
    def _get_air_absorption_coefficients(self, frequencies):
        """
