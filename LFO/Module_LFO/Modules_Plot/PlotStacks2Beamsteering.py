...
class StackDraw_Beamsteering:
    ...
    def draw_cabinet_based_stack(self, isrc, cabinet):
        """Zeichnet einen einzelnen Lautsprecher aus dem Stack (Frontansicht: width x frontheight)"""
        try:
            ...
            # Verwende frontheight aus Metadaten (keine Fallbacks)
            ...
            # Zeichne das Rechteck (Frontansicht: width x frontheight)
            # Normal: dunkle Fläche ist vorne (sichtbar) → dunkel
            # Cardio: helle Fläche/Body ist vorne (sichtbar) → hell
            rect = patches.Rectangle(
                (x, y),  # Position ist (x, base_y) - Rechteck geht nach oben
                width, 
                frontheight, 
                ec='black',
