...
PLOT_UPSCALE_FACTOR = int(os.environ.get("LFO_PLOT_UPSCALE_FACTOR", "1"))
...
class SPL3DPlotRenderer:
    ...
    def _filter_triangles_outside_surface(
        ...
        try:
            ...
            # 🎯 STRENGE FILTERUNG: Prüfe ALLE Eckpunkte jeder Zelle, nicht nur den Schwerpunkt
            # Eine Zelle wird nur behalten, wenn ALLE ihre Eckpunkte innerhalb des Polygons liegen
            # Das verhindert Artefakte an den Rändern, wo Dreiecke teilweise außerhalb liegen
            ...
            if vertical_orientation in ("xz", "xz_slanted"):
                # X-Z-Wand (senkrecht oder schräg): Prüfe in XZ-Ebene
                ...
                # Y-Z-Wand (senkrecht oder schräg): Prüfe in YZ-Ebene
            ...
            # Prüfe alle Punkte gleichzeitig
            ...
            # 🎯 STRENGE FILTERUNG: Entferne Zellen, wenn:
            # 1. NICHT ALLE Eckpunkte innerhalb sind ODER
            # 2. Der Schwerpunkt außerhalb ist (für zusätzliche Sicherheit)
            cells_to_keep = []
            
            # Berechne Schwerpunkte für alle Zellen
            try:
            ...
            # 🎯 STRENGE FILTERUNG: Für schräge/überhängende Flächen zusätzliche Prüfung
            ...
            for i in range(n_cells):
                try:
                    ...
                    # Prüfe ob ALLE Punkte der Zelle innerhalb sind
                    all_points_inside = np.all(points_inside_1d[cell_point_ids])
                    any_point_inside = np.any(points_inside_1d[cell_point_ids])
                    
                    # Prüfe ob Schwerpunkt innerhalb ist
                    centroid_inside = centroids_inside_1d[i] if i < len(centroids_inside_1d) else True
                    
                    # 🎯 VEREINFACHTE FILTERUNG: Da wir bereits vor der Triangulation filtern,
                    # sollten alle Punkte innerhalb des Polygons sein. Delaunay kann jedoch
                    # neue Punkte auf der konvexen Hülle erzeugen, die außerhalb des Polygons liegen.
                    # Daher prüfen wir: Behalte Zelle nur wenn ALLE Eckpunkte innerhalb sind.
            ...
            # Für Debug: Berechne auch Schwerpunkte (nur für Ausgabe)
            try:
                cell_centers = mesh.cell_centers().points
                centroids_3d = cell_centers
                if vertical_orientation in ("xz", "xz_slanted"):
    ...
    def update_spl_plot(
        ...
        # 🎯 DEBUG: Zeige verfügbare Daten im Container
        if DEBUG_PLOT3D_TIMING:
            print(f"[DEBUG Plot Datenfluss] Container-Daten geladen:")
            print(f"  └─ surface_grids_data: {len(surface_grids_data)} Surfaces")
            print(f"  └─ surface_results_data: {len(surface_results_data)} Surfaces")
            for sid in list(surface_grids_data.keys())[:3]:  # Zeige erste 3
                grid_data = surface_grids_data.get(sid, {})
                result_data = surface_results_data.get(sid, {})
                print(f"  └─ Surface '{sid}':")
                print(f"      └─ Grid Keys: {list(grid_data.keys())}")
                print(f"      └─ Result Keys: {list(result_data.keys())}")
                if 'X_grid' in grid_data:
                    x_grid = grid_data['X_grid']
                    if isinstance(x_grid, list):
                        if len(x_grid) > 0 and isinstance(x_grid[0], list):
                            print(f"      └─ X_grid: {len(x_grid)}x{len(x_grid[0])} (Listen)")
                        else:
                            print(f"      └─ X_grid: {len(x_grid)} Werte (Liste)")
        
        if not surface_grids_data or not surface_results_data:
            if DEBUG_PLOT3D_TIMING:
                print(f"[DEBUG Plot Datenfluss] ⚠️  Keine Grid- oder Result-Daten verfügbar!")
            return
        ...
        for surface_id in surface_grids_data.keys():
            ...
            try:
                ...
                # 🎯 DEBUG: Zeige geladene Grid-Daten
                if DEBUG_PLOT3D_TIMING:
                    print(f"[DEBUG Plot Datenfluss] Surface '{surface_id}': Grid-Daten geladen")
                    print(f"  └─ X_grid Shape: {X_grid.shape}, Type: {X_grid.dtype}")
                    print(f"  └─ Y_grid Shape: {Y_grid.shape}, Type: {Y_grid.dtype}")
                    print(f"  └─ Z_grid Shape: {Z_grid.shape}, Type: {Z_grid.dtype}")
                    print(f"  └─ sound_field_x: {len(sound_field_x)} Werte, Range: [{sound_field_x.min():.2f}, {sound_field_x.max():.2f}]")
                    print(f"  └─ sound_field_y: {len(sound_field_y)} Werte, Range: [{sound_field_y.min():.2f}, {sound_field_y.max():.2f}]")
                    print(f"  └─ X_grid Range: [{X_grid.min():.2f}, {X_grid.max():.2f}]")
                    print(f"  └─ Y_grid Range: [{Y_grid.min():.2f}, {Y_grid.max():.2f}]")
                    print(f"  └─ Z_grid Range: [{Z_grid.min():.2f}, {Z_grid.max():.2f}]")
                    print(f"  └─ surface_mask: {np.sum(surface_mask)}/{surface_mask.size} aktiv ({100*np.sum(surface_mask)/surface_mask.size:.1f}%)")
                
                # Lade SPL-Werte (komplex)
                sound_field_p_complex = np.array(result_data['sound_field_p'], dtype=complex)
                
                # 🎯 DEBUG: Zeige geladene SPL-Daten
                if DEBUG_PLOT3D_TIMING:
                    print(f"[DEBUG Plot Datenfluss] Surface '{surface_id}': SPL-Daten geladen")
                    print(f"  └─ sound_field_p_complex Shape: {sound_field_p_complex.shape}, Type: {sound_field_p_complex.dtype}")
                    valid_p = sound_field_p_complex[surface_mask]
                    if len(valid_p) > 0:
                        p_mag = np.abs(valid_p)
                        p_db = 20 * np.log10(np.maximum(p_mag, 1e-12))
                        print(f"  └─ SPL Range (dB): [{np.nanmin(p_db):.2f}, {np.nanmax(p_db):.2f}]")
                        print(f"  └─ SPL Mean (dB): {np.nanmean(p_db):.2f}")
